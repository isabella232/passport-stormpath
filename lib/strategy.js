...
function Strategy(o) {
  ...
  this.appHref = opts.appHref || process.env['STORMPATH_APP_HREF'] || '';

  var self = this;
  self.expansions = opts.expansions || null;
  self.accountStore = opts.accountStore || null;
...
Strategy.prototype.getApp = function(cb) {
    cb = cb || function() {};
    var self = this;
    ...
    if (self.spApp) {
        ...
        self.spClient.getApplication(self.appHref, function(err, app) {
            self.spApp = app;
...
Strategy.prototype.authenticate = function(req, options) {
  ...
  var accountStoreHref = (options.accountStore && options.accountStore.href) || (self.accountStore && self.accountStore.href);

  if (accountStoreHref) {
    data.accountStore = { href: accountStoreHref };
  }
  ...
  self.getApp(function(err, app) {
    if (err) {
      return self.fail({ message: err.userMessage }, err.status || 500);
    } else {
      app.authenticateAccount(data, function(err, authenticationResult) {
        if (err) {
          return self.fail({ message: err.userMessage }, err.status || 500);
        } else {
          var options = self.expansions ? { expand: self.expansions } : null;
